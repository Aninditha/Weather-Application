...
public class CityWeatherActivity extends AppCompatActivity implements GetData.Idata{
    ...
    String url,city, country, temp_unit = MainActivity.temp_unit;
    ProgressDialog pd;
    ...
    public void setList(final ArrayList<ArrayList<Weather>>list) {
        ...
        try{
            ...
            ApplySettings();
            dayDataRecyclerView.addOnItemTouchListener(new RecyclerItemClickListener(CityWeatherActivity.this,
                    hourlyDataRecyclerView, new RecyclerItemClickListener.OnItemClickListener() {
                @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.saveCity) {
            ...
            for (int i=0; i<savedCities.size();i++){
                if (savedCities.get(i).getCityName().equals(city)){
                    found= true;
                    index=i;
                    break;
                }
            ...
            if (found) {
                ...
                city1.setTemperature(String.valueOf(Math.round((((Double.parseDouble (
                        temp_list.get(0).get(0).getTemperature())-273.5)*100))/100)));
    ...
    public void ApplySettings(){
        ...
        if (!temp_unit.equals(temp_unit_change)){
            Toast.makeText(CityWeatherActivity.this,"Temperature unit change to "+temp_unit_change.toUpperCase(),Toast.LENGTH_SHORT).show();
        }
